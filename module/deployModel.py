...
class ImageProcessor:
    def __init__(self,
                 ...
                 detect_node_dilated: tuple=((3,3), 3),
                 gen_centers_min_area: float = 10.0,
                 group_points_by_y_threshold: float = 1.5,
    ...
    @staticmethod
    def cm_to_pixel(pixel_per_cm: float, distance_cm: float) -> float:
        """
        ...
        """
        return round(distance_cm*pixel_per_cm,2)

    def extract_roi(self, 
                    image: np.ndarray, 
                    size: tuple=(200, 1700, 200, 620), 
                    isShow: bool=False) -> np.ndarray:
        """
    ...
    def extract_maskNet(self, roi: np.ndarray,
    ...
    def detect_node(self, 
        ...
        # #xói mòn ảnh
        # dilatedImage = cv2.erode(dilatedImage, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,erode[0]), iterations = erode[1])
    ...
    def group_points_by_y(
        ...
        """
        Nhóm các điểm dựa trên tọa độ y (dựa trên hiệu số với điểm liền trước) 
        và vẽ các đường nối giữa các điểm cùng nhóm.

        Quy trình:
        1. Sắp xếp các điểm theo thứ tự tăng dần của tọa độ y (nếu y bằng nhau thì theo x).
        2. Duyệt qua danh sách: so sánh điểm hiện tại với điểm liền trước.
            Nếu hiệu số y nhỏ hơn hoặc bằng threshold thì thêm vào nhóm hiện tại,
            ngược lại, bắt đầu nhóm mới.
        3. Sắp xếp các điểm trong mỗi nhóm theo thứ tự tăng dần của x.
        ...
        Parameters:
            ...
            isShow (bool): Cờ xác định có hiển thị ảnh kết quả hay không.
            threshold (float): Ngưỡng cho phép sai lệch trên trục y giữa 2 điểm liên tiếp.
        ...
        # 1) Sắp xếp các điểm theo y tăng dần; nếu y bằng nhau thì sắp xếp theo x tăng dần.
        ...
        # 2) Gom nhóm bằng cách so sánh với điểm liền trước.
        for i in range(1, len(points)):
            prev_point = points[i - 1]
            current_point = points[i]
            diff_y = abs(current_point[1] - prev_point[1])
            if diff_y <= threshold:
                # Nếu hiệu số giữa điểm hiện tại và điểm liền trước nhỏ hơn hoặc bằng threshold,
                # thêm vào nhóm hiện tại.
                current_group.append(current_point)
            else:
                # Nếu không, lưu nhóm hiện tại và bắt đầu một nhóm mới.
        ...
        # 3) Sắp xếp từng nhóm theo thứ tự tăng dần của x.
        for group in groups:
            group.sort(key=lambda p: p[0])

        # 4) Vẽ các điểm và đường nối trong từng nhóm.
        for group in groups:
            # Bạn có thể dùng màu cố định hoặc sinh màu ngẫu nhiên cho mỗi nhóm.
            color = [np.random.randint(0, 255) for _ in range(3)]
            # Vẽ các đoạn thẳng nối điểm liền kề nếu nhóm có nhiều hơn 1 điểm.
            if len(group) > 1:
    ...
    def process(self, src, isLoadImg: bool = False, isShow: bool = False) -> tuple:
        ...
        corner_mask = self.detect_node(edges, self.detect_node_erode,
                                       opened=self.detect_node_opened,
                                       dilated=self.detect_node_dilated,
                                       isShow=True)
...
if __name__ == "__main__":
    folder_path = r"D:\DaiHoc\Intern\ThienPhuocCompany\data_fishNet\test"  # Thay đổi đường dẫn phù hợp
